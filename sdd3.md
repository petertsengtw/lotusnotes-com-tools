...
**查詢（同仁視角，加上身分驗證後）**
> 我從院內公告信件（或 QR code）加入 LINE@「花蓮職工福利行政小組」好友，開啟查詢頁後輸入我的姓名和信件裡的本期驗證碼。輸入正確就完成驗證，之後不管人在哪裡、用行動網路，只要透過這個 LINE@ 就能直接查特約商店優惠，不用再驗證第二次，除非驗證碼換期又被踢出名單。
...
| Notes 群組信廣播（沿用既有 Domino COM API，寄信給院內同仁群組信箱，非查詢個別通訊錄） | 定期分發本期驗證碼與 LINE@ 加入方式，只有能收到院內信件的人（即同仁）才拿得到本期的碼 |
...
| `active` | boolean | 目前是否為現行有效碼；換期時把舊碼設為 `false`，新增新碼 |
...
```mermaid
sequenceDiagram
    participant 福委會 as 福委會（Notes 共用帳號）
    participant Notes as Notes/Domino（既有 COM API）
    participant 同仁 as 同仁（手機／LINE@）
    participant Backend as 後端 API
    participant Firestore

    福委會->>Firestore: 產生本期驗證碼，寫入 verificationCodes
    福委會->>Notes: 寄信給院內同仁群組信箱（LINE@ QR code + 本期驗證碼）
...
- 廣播信件內容應提醒同仁「驗證碼請勿轉發給非本院同仁」，降低碼外流的風險（碼外流是殘餘風險，非目標是做到完全杜絕，見 §5.4 討論脈絡：真正的優惠兌現關卡仍在店家現場核對員工證）。

### 5.6 已知相依限制

- ~~福委會 Notes 帳號需具備寄送「院內同仁群組信」的權限~~ **已確認可行**：福委會共用 Notes 帳號可寄送全院公告，§5 設計的技術前提成立。尚待實作／測試的是「用 COM API 程式化觸發這個寄送動作」（沿用 `notes_store.py` 系列的連線方式），而不是這項權限本身是否存在。
- 換期驗證碼、觸發廣播信寄送，同樣受限於 Notes 無人值守排程的既有限制（見 §7 邊界情境、§8 已知問題 8.3）——代表每期換碼、寄信這個動作仍需要人工觸發或定期執行腳本，不是即時全自動。
...
| Notes ID 未開放「允許其他 Notes 程式使用此密碼」 | `Initialize()` 會跳出互動式密碼視窗，**無法無人值守排程**，必須手動在畫面上輸入密碼；此限制同時擋住 §5 驗證碼廣播信寄送的即時性（見 §5.6） |
...
| 8.3 | 目前無法排程，只能手動執行並手動輸入 Notes 密碼；§5 的驗證碼換期與廣播信寄送也會受同樣限制影響 | 中（資料新鮮度、驗證碼換期即時性都依賴人工記得執行） | 待 Notes ID 開放「允許其他程式使用此密碼」後，可比照自動打卡功能的排程模式（Windows 工作排程器）自動化 |
...
- ~~§5.5：驗證碼的實際輪替頻率、由誰觸發？~~ **已確認：由維運者本人手動觸發換期**，無固定排程頻率，沿用 `deploy_store.py` 的手動執行模式（見 §5.6、8.3 的 Notes 排程限制，換碼與寄送廣播信同樣是手動流程）。
